...
class FriendCloset :  BaseActivity<ActivityFriendClosetBinding>(ActivityFriendClosetBinding::inflate) {
    ...
    private fun init() {
        setToolbar()
        // 첫 화면 세팅
        setFragment(FriendFragment(), "clothes")
        ...
        binding.bottomNavFriend.setOnItemSelectedListener { item ->
            when(item.itemId) {
                ...
                R.id.nav_friend_style -> {
                    setFragment(FriendFragment(), "style")
                    true
                }
                else -> false
            }
    ...
    private fun setToolbar() {
        setSupportActionBar(binding.toolbarFriend.toolbar)
        ...
        supportFragmentManager.beginTransaction().replace(R.id.frameLayout_friend_container, fragment, tag).commit()
    }
                ...
                when(getCurrentFragmentTag()) {
                    "clothes" -> {
                        object: View.OnClickListener {
                            override fun onClick(v: View?) {
                                startActivity(Intent(this@FriendCloset, PhotoSelectActivity::class.java).apply {
    ...
    private fun getCurrentFragmentTag(): String? = supportFragmentManager.findFragmentById(R.id.frameLayout_friend_container)?.tag
    ...
    private fun setLeadingIcon(resId: Int, clickListener: View.OnClickListener) {
        try {
            binding.toolbarFriend.imageViewFriendToolbarLeadingIcon.background = ResourcesCompat.getDrawable(resources, resId, null)
        } catch (e: Exception) {}
    ...
    private fun setTrailingIcon(resId: Int, clickListener: View.OnClickListener) {
        try {
            binding.toolbarFriend.imageViewFriendToolbarTrailingIcon.background = ResourcesCompat.getDrawable(resources, resId, null)
        } catch (e: Exception) {}
