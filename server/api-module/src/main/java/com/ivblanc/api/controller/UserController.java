...
public class UserController {
    ...
    public @ResponseBody CommonResult userUpdatePw(@Valid UpdatePwReqDTO req) throws Exception{
        // 존재하는 회원인지 확인
        User user = userService.getUserDetailById(req.getUser_id());
        if(user == null)
        ...
        User res = userService.updatePassword(user.getUser_id(), passwordEncoder.encode(req.getPassword_new()));
        if(res == null)
    ...
    public @ResponseBody CommonResult userUpdatePersonal(@Valid UpdatePersonalReqDTO req) throws Exception{
        ...
        User res = userService.updatePersonal(user.getUser_id(), req.getAge(), req.getGender(), req.getPhone());
        if(res == null)
    ...
    public @ResponseBody CommonResult userSignOut(@Valid SignOutReqDTO req) throws Exception{
        // 존재하는 회원인지 확인
        User user = userService.getUserDetailByEmail(req.getEmail());
        if(user == null)
            throw new ApiMessageException("등록되지 않은 회원입니다.");
        ...
        int res = userService.deleteUser(user.getUser_id());
        if(res == 0)
            throw new ApiMessageException("회원 탈퇴에 실패했습니다. 다시 시도해 주세요.");
